<div role="card"
    class="relative inline-block w-72 mx-10 my-10 text-sm text-gray-500 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg p-4">
    ...
    <div class="flex items-center justify-between mb-3">
        ...
        <form action="{{ route('profiles.follow', $profile->id) }}" method="POST">
            @csrf
            <button type="submit"
                class="text-white {{ Auth::check() && Auth::guard('web')->user()->isFollowing($profile) ? 'bg-red-700 hover:bg-red-800' : 'bg-blue-700 hover:bg-blue-800' }} focus:ring-4 focus:ring-blue-300 dark:focus:ring-blue-600 font-medium rounded-lg text-xs px-3 py-1.5 {{ Auth::check() && Auth::guard('web')->user()->isFollowing($profile) ? 'hover:text-white' : 'text-blue-500 hover:text-white' }}">
    ...
    <!-- Username (Email Link) -->
    <p class="text-sm font-normal text-gray-600 dark:text-gray-400 mb-2">
    ...
    <ul class="flex text-sm text-gray-900 dark:text-gray-200">
        <li class="mr-4">
